-- Migration 065: Make lampiran_tarif nodes searchable via FTS/trigram
--
-- The Lampiran chunking fix (scripts/loader/load_to_supabase.py:_split_lampiran_sections)
-- splits tariff appendices (Lampiran I/II/III) into per-table `lampiran_tarif` nodes instead
-- of one 182k-char `penjelasan_pasal` blob. search_legal_chunks() filters to a fixed set of
-- content-bearing node types (v_node_types) and did NOT include `lampiran_tarif`, so those
-- nodes would never surface via FTS/trigram. This adds `lampiran_tarif` to that set.
--
-- match_legal_chunks() (vector RPC, migration 062) needs NO change: it filters only on
-- `embedding IS NOT NULL`, so embedded lampiran_tarif nodes are already returned by vector search.
--
-- Signature is unchanged -> plain CREATE OR REPLACE (no DROP, no window). Body is identical to
-- migration 063 except the single v_node_types array literal.
--
-- APPLY ORDER: apply this BEFORE re-ingesting Perda 1/2024 (the re-ingest is what creates the
-- lampiran_tarif nodes). Applying it earlier is a harmless no-op (no node has that type yet).
