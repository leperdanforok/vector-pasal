-- Migration 056: Hybrid Search — FTS + trigram Reciprocal Rank Fusion (RRF)
--
-- Adds a hybrid search layer that combines FTS ranking with pg_trgm
-- word similarity scoring. Zero API cost — both signals are computed
-- entirely within PostgreSQL.
--
-- How it works:
--   1. Run existing FTS (websearch_to_tsquery) to get candidates
--   2. For each candidate, also compute word_similarity() score
--   3. Combine via RRF: 1/(k+fts_rank) + 1/(k+trgm_rank)
--   4. Re-order by combined RRF score
--
-- This catches near-misses that FTS stemming loses (e.g., abbreviations,
-- partial matches, OCR-mangled terms) without any embedding API calls.
--
-- Also adds an optional `embedding` column to document_nodes for future
-- vector search integration (populated by load_perda_bolmong.py).

-- ============================================================
-- Step 1: Add embedding column for future vector search
-- Uses vector(768) to match Gemini embedding-001 output dimensions.
-- Nullable — search works without it. Only populated for regulations
-- that go through the embedding pipeline.
-- ============================================================

DO $$
BEGIN
    -- Only add if pgvector extension is available and column doesn't exist
    IF EXISTS (SELECT 1 FROM pg_extension WHERE extname = 'vector') THEN
        IF NOT EXISTS (
            SELECT 1 FROM information_schema.columns
            WHERE table_name = 'document_nodes' AND column_name = 'embedding'
        ) THEN
            EXECUTE 'ALTER TABLE document_nodes ADD COLUMN embedding vector(768)';
            EXECUTE 'CREATE INDEX IF NOT EXISTS idx_nodes_embedding ON document_nodes USING ivfflat (embedding vector_cosine_ops) WITH (lists = 100)';
        END IF;
    END IF;
END $$;

-- ============================================================
-- Step 2: Replace search_legal_chunks() with hybrid FTS+trigram version
-- Same function name, same params, same return shape — zero consumer changes.
-- ============================================================
...
BEGIN
    -- Parse comma-separated type filter into array
    v_type_filters := CASE
    ...
    -- Parse comma-separated status filter into array
    ...
    -- ================================================================
    -- Layer 3: HYBRID — FTS + trigram Reciprocal Rank Fusion (RRF)
    --
    -- Combines ts_rank_cd (stemmed term matching) with word_similarity
    -- (character n-gram matching) using RRF with k=60.
    -- FTS catches exact legal terms; trigram catches near-misses,
    -- abbreviations, and OCR-mangled text.
    -- Zero API cost — both signals are PostgreSQL built-ins.
    -- ================================================================
    ...
    IF v_tsquery IS NOT NULL THEN
        ...
        scored AS (
            SELECT
                c.*,
                -- FTS score with hierarchy and recency boosts
                (
                    ts_rank_cd(c.fts, v_tsquery)
                    * (1.0 + (10 - COALESCE(c.rt_level, 5)) * 0.05)
                    * (1.0 + GREATEST(0, COALESCE(c.w_year, 2000) - 1990) * 0.005)
                )::float AS fts_score,
                -- Trigram word similarity (0.0 – 1.0, higher = more similar)
                word_similarity(v_safe, LEFT(c.content_text, 2000))::float AS trgm_score
            FROM candidates c
        ),
        ranked AS (
            SELECT
                s.*,
                -- Reciprocal Rank Fusion: combines FTS and trigram rankings
                -- RRF(d) = 1/(k + rank_fts) + 1/(k + rank_trgm)
                -- k=60 is the standard constant from the original RRF paper
                (
                    1.0 / (60 + ROW_NUMBER() OVER (ORDER BY s.fts_score DESC))
                    + 0.4 / (60 + ROW_NUMBER() OVER (ORDER BY s.trgm_score DESC))
                )::float AS rrf_score
            FROM scored s
        )
        SELECT
    ...
    IF v_count = 0 THEN
        -- Tier 2: plainto_tsquery + trigram RRF
...
-- Re-apply search_path hardening from migration 049
