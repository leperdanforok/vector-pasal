-- Migration 059: Rebuild search_legal_chunks with OR-based FTS + Sanction Expansion
--
-- Root cause fixed: the previous function used AND-logic FTS, requiring ALL query
-- terms to appear in a SINGLE node. Legal questions span multiple nodes:
--   - The violation (e.g., Pasal 21: "dilarang membuang sampah di sungai")
--   - The penalty  (e.g., Pasal 61: "denda paling banyak Rp 20.000.000,- ")
-- These never share all keywords, so AND-FTS always returned empty.
--
-- Solution:
--   1. OR-based FTS: any significant word can match (catches violation nodes).
--   2. Sanction expansion: always include top denda/pidana nodes from matched works.
--   3. Trigram secondary ranking for typo tolerance via RRF.
--
-- Also adds trigram index for fast word_similarity() lookups.

-- 1. Ensure trigram index exists
