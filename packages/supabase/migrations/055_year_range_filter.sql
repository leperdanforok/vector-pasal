-- Migration 055: Add year_from range filter to search_legal_chunks()
--
-- Adds v_year_from variable alongside existing v_year_filter (exact match).
-- Each WHERE clause gets: AND (v_year_from IS NULL OR w.year >= v_year_from)
--
-- Backward compatible: existing consumers don't pass year_from, so v_year_from
-- is NULL and the new clause is skipped.
