...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const url = `https://vector-pasal${path}`;
  ...
  const ogImageUrl = `https://vector-pasal/api/og?${ogParams.toString()}`;
...
async function LawReaderSection({
  ...
  const pageUrl = `https://vector-pasal/peraturan/${type.toLowerCase()}/${slug}`;
...
export default async function LawDetailPage({ params }: PageProps) {
  ...
  const breadcrumbLd = {
    ...
    "@type": "BreadcrumbList",
    itemListElement: [
      { "@type": "ListItem", position: 1, name: t("breadcrumbHome"), item: "https://vector-pasal" },
      { "@type": "ListItem", position: 2, name: type.toUpperCase(), item: `https://vector-pasal/jelajahi/${type.toLowerCase()}` },
