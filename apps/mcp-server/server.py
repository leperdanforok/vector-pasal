"""vectorpasal.vercel.app MCP Server — Indonesian Legal Database (v0.3).
...
DISCLAIMER = (
    ...
    "Database vectorpasal.vercel.app saat ini mencakup sebagian kecil peraturan Indonesia."
)
...
mcp = FastMCP(
    "vectorpasal.vercel.app — Indonesian Legal Database",
    instructions=(
...
def ping() -> str:
    ...
    try:
        result = sb.table("works").select("id", count="exact").execute()
        count = result.count or 0
        return f"vectorpasal.vercel.app MCP server is running. Database has {count} laws loaded."
    except Exception as e:
